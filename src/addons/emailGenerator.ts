...
class EmailGenerator {
                                            ...
                                            <tbody>
                                                <tr>
                                                    <td style="font-family:'Open Sans', Arial, sans-serif; font-size:11px; line-height:18px; color:#999999;"
                                                        ...
                                                        <br>
                                                        For any other issues, Please contact Anshul (anshul.lalit@kdi.kongsberg.com)
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
    ...
    msToHMS = (ms: number) => {
        let seconds = ms / 1000,// 1- Convert to seconds
            hours = Math.round(seconds / 3600); // 2- Extract hours: 3,600 seconds in 1 hour

        seconds = seconds % 3600; // seconds remaining after extracting hours

        let minutes = Math.round(seconds / 60); // 3- Extract minutes: 60 seconds in 1 minute
    ...
    longBar = (character: string, length: number) => {
        let myLength = Math.round(length), mybar = "";
        for (let i = 0; i < myLength; i++) mybar = mybar + character;
        return mybar;
    }
};

let generator = new EmailGenerator(), executionSummary, abandonedText;
...
let outputLoc = "build-js/emailReport.html"
